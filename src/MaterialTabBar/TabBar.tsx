...
const MaterialTabBar = <T extends TabName = TabName>({
  ...
  const [itemsLayout, setItemsLayout] = React.useState<ItemLayout[]>(
    scrollEnabled
      ? []
      : tabNames.map((_, i) => {
        const tabWidth = showDefaultTabs ? width / nTabs : nTabs != 2 ? ((70 * width) / 100) : (43 * width) / 100
  ...
  return (
    ...
    >
      ...
      }
      {
        ...
        <Animated.View style={[styles.outerTabContainer, { width: '80%', justifyContents: 'center' }, RF && { height: RF(44) }]}>
          {
